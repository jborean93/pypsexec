...
class TestDataRepresentationFormat(object):
    ...
    def test_create_message(self):
        ...
        expected = b"\x10" b"\x00" b"\x00" b"\x00"
        actual = message.pack()
    ...
    def test_parse_message(self):
        ...
        data = b"\x10" b"\x00" b"\x00" b"\x00"
        data = actual.unpack(data)
        assert len(actual) == 4
        assert data == b""
        assert (
            actual["integer_character"].get_value()
            == IntegerCharacterRepresentation.ASCII_LITTLE_ENDIAN
        )
        assert actual["floating_point"].get_value() == FloatingPointRepresentation.IEEE
...
class TestContextElement(object):
    ...
    def test_parse_message(self):
        ...
        assert isinstance(actual["abstract_syntax"].get_value(), SyntaxIdElement)
        assert actual["abstract_syntax"]["uuid"].get_value() == uuid.UUID(
            bytes=b"\xff" * 16
        )
        ...
        assert transfer_syntaxes[0]["uuid"].get_value() == uuid.UUID(bytes=b"\xee" * 16)
        assert transfer_syntaxes[0]["version"].get_value() == 0
        assert transfer_syntaxes[1]["uuid"].get_value() == uuid.UUID(bytes=b"\xdd" * 16)
...
class TestResult(object):
    ...
    def test_parse_message(self):
        ...
        assert actual["reason"].get_value() == ResultReason.REASON_NOT_SPECIFIED
        assert actual["transfer_syntax"]["uuid"].get_value() == uuid.UUID(
            bytes=b"\xff" * 16
        )
...
class TestBindPDU(object):
    ...
    def test_parse_message(self):
        ...
        assert (
            actual["packed_drep"]["integer_character"].get_value()
            == IntegerCharacterRepresentation.ASCII_LITTLE_ENDIAN
        )
        assert (
            actual["packed_drep"]["floating_point"].get_value()
        ...
        assert context_elems[0]["abstract_syntax"]["uuid"].get_value() == uuid.UUID(
            bytes=b"\xff" * 16
        )
        ...
        assert transfer_syntaxes[0]["uuid"].get_value() == uuid.UUID(bytes=b"\xff" * 16)
        assert transfer_syntaxes[0]["version"].get_value() == 0
...
class TestBindAckPDU(object):
    ...
    def test_create_message(self):
        ...
        message["assoc_group_id"] = 2
        message["sec_addr"] = (
            b"\x5c\x70\x69\x70\x65\x5c\x6e\x74" b"\x73\x76\x63\x73\x00"
        )
    ...
    def test_parse_message(self):
        ...
        assert (
            actual["sec_addr"].get_value() == b"\x5c\x70\x69\x70\x65\x5c\x6e\x74"
            b"\x73\x76\x63\x73\x00"
        )
        ...
        assert results[0]["reason"].get_value() == ResultReason.REASON_NOT_SPECIFIED
        assert results[0]["transfer_syntax"]["uuid"].get_value() == uuid.UUID(
            bytes=b"\xff" * 16
        )
        assert results[0]["transfer_syntax"]["version"].get_value() == 2
        assert results[1]["result"].get_value() == ContextResult.NEGOTIATE_ACK
        assert results[1]["reason"].get_value() == ResultReason.LOCAL_LIMIT_EXCEEDED
        assert results[1]["transfer_syntax"]["uuid"].get_value() == uuid.UUID(
            bytes=b"\xff" * 16
        )
...
class TestBindNakPDU(object):
    ...
    def test_parse_message(self):
        ...
        assert actual["call_id"].get_value() == 4
        assert (
            actual["provider_reject_reason"].get_value()
...
class TestFaultPDU(object):
    ...
    def test_parse_message(self):
        ...
        assert actual["reserved"].get_value() == 0
        assert actual["status"].get_value() == FaultStatus.NCA_S_FAULT_ADDR_ERROR
