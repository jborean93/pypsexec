...
class TestClient(object):
    ...
    def test_proc_both_elevated_and_limited_error(self):
        ...
        assert (
            str(exc.value) == "Both run_elevated and run_limited are "
    ...
    def test_proc_stdin_and_async(self):
        ...
        assert (
            str(exc.value) == "Cannot send stdin data on an interactive "
...
class TestClientFunctional(object):
    ...
    def test_proc_long_running(self, client):
        arguments = "Write-Host first; Start-Sleep -Seconds 15; " "Write-Host second"
    ...
    def test_proc_custom_stdout_stderr(self, client):
        ...
        arguments = "/c echo first && echo second 1>&2 && echo third"
        actual = client.run_executable(
            "cmd.exe", arguments=arguments, stdout=OutputPipeList, stderr=OutputPipeList
        )
        assert actual[0] == [b"first \r\n", b"third\r\n"]
        assert actual[1] == [b"second  \r\n"]
        assert actual[2] == 0
    ...
    def test_proc_limit_processor(self, client):
        actual = client.run_executable(
            "powershell.exe", arguments="Write-Host hello", processors=[1]
        )
    ...
    def test_proc_dont_load_profile(self, client):
        actual = client.run_executable(
            "powershell.exe", arguments="Write-Host done", load_profile=False
        )
    ...
    def test_proc_interactive(self, client):
        actual = client.run_executable(
            "powershell.exe", arguments="Write-Host done", interactive=True
        )
    ...
    def test_run_specific_user(self, client):
        ...
        actual = client.run_executable(
            "whoami.exe", arguments="/groups", username=username, password=password
        )
    ...
    def test_run_invalid_user(self, client):
        with pytest.raises(PAExecException) as exc:
            client.run_executable(
                "whoami.exe", username="fakeuser", password="fakepassword"
            )
        assert (
            str(exc.value) == "Received exception from remote PAExec "
    ...
    def test_proc_with_working_dir(self, client):
        actual = client.run_executable(
            "cmd.exe", arguments="/c cd", working_dir="C:\\Windows"
        )
    ...
    def test_proc_with_timeout(self, client):
        ...
        actual = client.run_executable(
            "powershell.exe", arguments="Start-Sleep -Seconds 20", timeout_seconds=5
        )
    ...
    def test_proc_with_stdin(self, client):
        actual = client.run_executable(
            "powershell.exe", arguments="-", stdin=b"Write-Host input\r\nexit 0\r\n"
        )
    ...
    def test_proc_with_stdin_generator(self, client):
        ...
        actual = client.run_executable(
            "powershell.exe", arguments="-", stdin=stdin_generator
        )
    ...
    def test_proc_with_async(self, client):
        start_time = time.time()
        actual = client.run_executable(
            "powershell.exe", arguments="Start-Sleep -Seconds 10", asynchronous=True
        )
        actual_time = time.time() - start_time
    ...
    def _get_paexec_files_and_services(self, client):
        ...
        client = Client(server, username=username, password=password)
        client.connect()
        scmr = client._service.scmr
        scmr_handle = client._service.scmr_handle
        ...
        smb_tree = TreeConnect(
            client.session, r"\\%s\ADMIN$" % client.connection.server_name
        )
        smb_tree.connect()
    ...
    def _get_new_generic_client(self, client):
        ...
        new_client.service_name = "PAExec-%d-%s" % (
            new_client.pid,
            new_client.current_host,
        )
        new_client._exe_file = "%s.exe" % new_client.service_name
        new_client._service = Service(new_client.service_name, new_client.session)
        return new_client
