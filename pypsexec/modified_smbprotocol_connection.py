...
from pypsexec.modified_smbprotocol_transport import (
    Tcp,
)
...
class Connection(object):
    ...
    def __init__(self, guid, server, require_signing=True):
        """
        [MS-SMB2] v53.0 2017-09-15
        ...
        :param server: The server to start the connection
        ...
        """
        self.server = server
        self.server_name = server.getpeername()[0]
        ...
        log.info("Initialising connection, guid: %s, require_singing: %s, "
                 "server: %s, port: %d"
                 % (guid, require_signing, self.server_name, self.port))
    ...
    def connect(self, dialect=None, timeout=60):
        ...
        message_queue = Queue()
        self.transport = Tcp(self.server, message_queue, timeout)
